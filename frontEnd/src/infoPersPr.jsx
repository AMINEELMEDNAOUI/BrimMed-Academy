...
const InfoPersPr = () => {
    ...
    const fetchStudents = (userId) => {
        axios.get(`http://localhost:8080/api/professors/matiere/${userId}`)
            .then(response => {
            ...
            <div className="card-container">
                {listeEtud.map((etud, index) => (
                    <div className="student-card" key={index}>
                        <div className='sdv'>
                            <h1 className='h3cardst'>Pr. {etud.nom} {etud.prenom}</h1>
                            <p className='pemst'><strong>Email:</strong> {etud.mail}</p>
                            <p className='plevst'><strong>Level:</strong> {etud.niveau}</p>
                            <p className='psubst'><strong>Subject:</strong> {etud.matieres.join(', ')}</p>
                        </div>
